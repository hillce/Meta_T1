...
datasetTrain = Train_Meta_Dataset(modelDir=modelDir,fileDir=fileDir,t1MapDir=t1MapDir,size=10000,transform=trnsInTrain,load=load)
datasetVal = Val_Meta_Dataset(modelDir=modelDir,fileDir=fileDir,t1MapDir=t1MapDir,size=1000,transform=trnsInVal,load=load)
datasetTest = Test_Meta_Dataset(modelDir=modelDir,fileDir=fileDir,t1MapDir=t1MapDir,size=1000,transform=trnsInVal,load=load)
...
for nE in range(numEpochs):
    ...
    with torch.no_grad():
        ...
        for i,data in enumerate(loaderVal):
            ...
            sys.stdout.write("\r\tSubj {}/{}: Loss = {:.5f}".format(i*bSize,valLen,err.item()))
        ...
        if valLoss < lowestLoss:
            ...
            "lr_scheduler":lrSchedulerG.state_dict()
